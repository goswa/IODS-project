...
If we compare wide and long form data, the wide form combines one of the keys with the value variables and their columns represent groups. The wide data has multiple columns for outcomes as we can see in RATS and BPRS dataset. The long form data presents each subject's response in multiple rows and every row shows a response/observation that belongs to a particular variable. In simple terms, the long form represents columns of outcomes into a single response variable as evident in BPRSL and RATSL dataset. Thus, for regression function, long form data is preferred.
...
Hypothesis: The growth profiles of three groups of rats differ with time.
...
Graphically, there is an obvious difference in weight growth profile of individual rats in three groups. Group 1 seems to be under 300 grams. Group 2 lies between 400 to approx 600 grams. Group 2 seems to have highest growth. The graph is incresing with time. In Group 3, some subjects have higher growth profile but some seems to have decreased growth with time. Group 1 seems to have slight growth change throughout the study.
...
The rats who has more weight at the beginning seems to have increasing weight profile throughout the study.To see it in more clear way, standardizarion is done. The strandardization includes the values obtained by subtracting the relevant occasion mean from the original observation and then dividing by the corresponding visit standard deviation. There seems to be an incresing trend in all the three groups. Some rat's growth are increasing ezponentially. Group 2 seems to have highest growth rate.
...
The objective of the ANOVA test is to analyse if there  is a (statistically) significant difference in weight growth between different groups. It is going to compare means of response among the different groups and check if differences are statistically significant. Two hypothesis are there in ANOVA:
          1. Null Hypothesis: All group means are equal and there is no difference among them.
          2. Alternative Hypothesis: All group means are NOT equal and there is difference among the three groups under observation.
...
Hence the above mentioned hypothesis is true and growth profiles of three groups of rats differ with time.
...
There seems to have overlapping between two treatments in the first plot. In the second plot, there seems to have decreasing trend in bprs score over the 8 weeks of time. 
...
A multiple linear regression model is fitted with bprs score as response and week and treatment as explanatory variables.
